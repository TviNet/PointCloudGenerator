...
if (__name__ == "__main__"):
    xml_path = sys.argv[sys.argv.index("--") + 1]
    ...
    scale = 10
    obj_scale = 10
    #xml_path = './opendrive_files/road_specification_v3.xodr'
    tree = ET.parse(xml_path)
    
    # selection
    bpy.data.objects['Cube'].select = True
    bpy.data.objects['Camera'].select = False
    print('Processing file ', xml_path)
    # remove it
    ...
    for road in root.findall('road'):
        ...
        for signal in current_road.signals:
            ...
            try:
                ...
                print('Could not place : ', signal.name)
            signal_counter += 1
        ...
        if currentRoad.length/100 < 1:
            for s in np.arange(0, currentRoad.length, currentRoad.length/100):
                ...
                if(pt != None):
                    ...
                    allverts.append(generate_lane_mark_verts(pt,tangent,lane_data, 0.12))
            ...
            for s in np.arange(0, currentRoad.length, 1):
                lane_data = currentRoad.get_lane_data(s)
                currentRoad.lane_sections[0].lanes
                pt,tangent = currentRoad.get_pt_tangent(s,0)
                elev = currentRoad.get_elevation(s)
    ...
    mesh_name = os.path.basename(xml_path)[:-5]
    bpy.ops.wm.save_as_mainfile(filepath=f"./files/blend/{mesh_name}.blend")
